...
| POST | `/api/practice/questions/:questionId/submit` | سؤال MCQ: `{optionIndex}` → `{correct: boolean, explanation: string|null, mastery: {score, decayedScore, level, labelAr}|null}` (null عند سؤال بلا مفهوم)؛ التصحيح يعتمد `correctIndex` خادميًا ويُدرّب التقييم — **PHASE 26: الدلتا حسب `difficulty`** (`+0.15`/`−0.1` سهل، `+0.175`/`−0.125` متوسط، `+0.2`/`−0.15` صعب على صف موجود؛ `0.6/0.1` أول محاولة محايدة للصعوبة). سؤال **مفتوح (PHASE 30)**: `{answer}` (نص حر ≤ `1500` حرفًا) → `{correct, score: number|null, feedback: string|null, explanation: string|null, mastery}` عبر عملية LLM `grade_open` (`<reference>` نظامًا من `answerKey`/<`student_answer>` مستخدمًا؛ حارس لا-نص-حرفي + سقوط قوَالبي حتمي — التغذية **بلا مفتاح أبدًا**). كلا النوعين يكتب صف `answers` (بـ`content` = الخيار المختار أو النص الحر) ويقيّم شارات التمرين/الإتقان (best-effort) | student فقط |
