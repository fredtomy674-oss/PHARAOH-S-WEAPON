...
> آخر تحديث: 2026-09-25 — التنفيذ في `server/test/` (Vitest **254/254**)، الويب يُفحص بنيويًا + E2E بالمتصفح **33/33**.
...
> آخر تحديث: 2026-09-25 — **33/33 أخضر** عبر `npm run e2e`. الاختبارات حقيقية 100%: متصفح Chromium → React SPA → Vite proxy → Fastify → SQLite → RAG → AI provider (mock=افتراضي المشروع) → persistence → المتصفح.
...
npm run e2e                       # يشغّل كل شيء تلقائيًا (خادمان مُداران + 33 اختبارًا)
