...
> آخر تحديث: 2026-09-24 — التنفيذ في `server/test/` (Vitest **208/208**)، الويب يُفحص بنيويًا + E2E بالمتصفح **30/30**.
...
> آخر تحديث: 2026-09-24 — **30/30 أخضر** عبر `npm run e2e`. الاختبارات حقيقية 100%: متصفح Chromium → React SPA → Vite proxy → Fastify → SQLite → RAG → AI provider (mock=افتراضي المشروع) → persistence → المتصفح.
...
npm run e2e                       # يشغّل كل شيء تلقائيًا (خادمان مُداران + 30 اختبارًا)
