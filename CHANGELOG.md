...
## 2026-09-25 — الجلسة الثالثة والعشرون (PHASE 29: ملخص الجلسة الآمن — إغلاق بند D-027 «مُلخّص جلسة AI آمن للعرض»)
- **القرار D-033**: المدرّس لا ينهي درسًا بلا محصّلة — عملية LLM ديناميكية **`recap`** (خارج `LLM_CACHEABLE` مثل tutor/feedback) تُبنى من **بيانات وصفية فقط** (عنوان الدرس، أسماء المفاهيم، **عدادات فقط**: رسائل الطالب/المدرّس، المدة، المرفقات، أعلام الأمان) — محتوى الرسائل **لا يدخل الاستدعاء ولا النتيجة**: «لا تسريب» **بالتصميم**، لا بمجرد «طلب النموذج ألّا يعيد النص». حارس **«لا نص حرفي»** (`recapContainsMessageContent` — اتجاهان: رسالة كاملة ≥12 حرفًا داخل الملخص، وجملة ملخص ≥25 حرفًا داخل رسالة، مع استبعاد عناوين الدرس/المفاهيم كرموز مسموحة) يرفض أي ناتج يعيد إنتاج رسالة → **سقوط آمن حتمي** `buildRecapFallback` (`fallback:true`) — فيرى المستخدم دائمًا ملخصًا آمنًا. صفر مخطط جديد.
- **الختم الحتمي**: `sessions/recap.ts` (جديد — `buildRecapMetadataBlock`/`parseRecap`/`buildSessionRecap`/`buildRecapFallback`) + mock `extractMetadataBlock` + `buildMockRecap(block)` حتمي (قوالِب تُنتقى من العدادات — بلا محتوى؛ نموذج `mock-recap`).
- **الوجهان**: الطالب `GET /api/sessions/:id/recap` (student فقط — غير طالب 403، جلسة الآخرين 403 ملكية، بلا رسائل → `{recap:null}`) والوالد `GET /api/parent/children/:studentId/sessions/:sessionId/recap` (بوابة `students_parents` → 404 قبل القراءة؛ **نفس الحمولة** — لا يرى الوالد أكثر مما يرى الطالب؛ `contextUserId` لولي الأمر).
- **الويب**: مكوّن مشترك `web/src/RecapCard.tsx` — في الرئيسية زر **«ملخص الجلسة»** على صف الجلسة المنتهية بالذات (القائمة تصاعدية → استهداف الصف بمعرّفه) يفتح البطاقة (عنوان/تركيز/إحصائيات/نقاط قوة/اقتراحات/تنبيه تلقائي)، وفي لوحة الوالد بطاقة **«ملخص الجلسة الآمن»** تُحمَّل عند الطلب فقط.
- **اختبارات**: وحدة `recap.test.ts` **+18** (بيانات وصفية/حدود الباحث/حارس بكل الاتجاهات/دمج وسقوطان/حتمية) + API `sessionRecap.test.ts` **+8** (حتمية لنفس الجلسة، عنوان درس، **لا رسالة سرية في العرض**، `null` بلا رسائل، عزل 403، غير طالب 403، والد مربوط وغير مربوط/403) → **315/315 أخضر**؛ E2E `recap.spec.ts` **R1** (طالب: درس → رسالة سرية → «ملخص الجلسة» على صفّه → عنوان درسه بلا نص الرسالة) و**R2** (والد: أحدث جلسة → «عرض الملخص» → نفس الضمانة) → **38/38 أخضر**.
- `npm run check` أخضر (315/315) + `npm run build` أخضر + docs sync (DECISIONS D-033/TASKS/CHANGELOG/TEST_PLAN/README/API_SPEC) + commit.

## 2026-09-25 — الجلسة الرابعة والعشرون (PHASE 30: التصحيح الآلي للإجابات المفتوحة — إغلاق آخر بند D-028)
- **القرار D-034**: إجابة الطالب المكتوبة بجملته تُصدَّر الآن عبر تصحيح LLM **ديناميكي** `grade_open` — تفعيل أسئلة `type:"open"` (عمود `answerKey` الخامل منذ PHASE 24): `answerKey` يبقى **خادميًا فقط** ويصل لرسالة النظام (`<reference>`) بينما إجابة الطالب في رسالة المستخدم (`<student_answer>`)؛ حارس «لا نص حرفي» (`gradeContainsAnswerKey` — حرفي ≥3 أحرف أو ≥60% تغطية) + **سقوط قوَالبي حتمي** (`gradeFallback` بلا إعادة المفتاح) + سقف `GRADE_OPEN_MAX_STUDENT_ANSWER_CHARS=1500`. الصحيح ≥0.7/القريب ≥0.4 — mock حتمي بتغطية التوكنات.
- **العزل**: `practice/grade.ts` (جديد — `normalizeArabic` بتحويل الأرقام العربية/الهندية/توحيد الألف، `tokensOf`، `referenceCoverage`، `gradePrompt`، `parseGrade` صارم، حارس، سقوط، ثوابت) + `AIOperation += grade_open` (**خارج `LLM_CACHEABLE`** — كل تصحيح يُكلَّف؛ `CACHEABLE_OPERATIONS` الإدارية لم تتغيّر).
- **النوع في كل المسارات**: `submit` يوزّع بطبقة النوع (`INVALID_ANSWER` لخيار على مفتوح / `INVALID_OPTION` لنص على mcq / `INVALID_SUBMIT` لحمولة مختلطة / `INVALID_TYPE`/`INVALID_KIND` للمعلمات)؛ التوليد الذاتي والإداري بأصناف `kind:"mcq"|"open"` مع حارس عداد لكل نوع (→ 409)؛ الخطة تضيف `openQuestions`.
- **البذر الحتمي**: «تبسيط الكسور» (`answerKey` «2/3») و«القسمة المطولة» («26») — سؤالا E2E مضمونا النتيجة.
- **الويب**: صف الخطة يكتسب «سؤال مقالي» (`plan-open-practice`) أو «توليد سؤال مقالي» (`plan-open-generate`)؛ اللوحة تعرض حقل النص الحر `practice-open-input` وتغذية راجعة `feedback` + شارة درجة `practice-score`.
- **اختبارات**: وحدة `grade.test.ts` **+27** + API `openQuestion.test.ts` **+11** (لا تسريب مفتاح في أي استجابة، 409/400/403، صحيح وخاطئ عبر مفتاح DB) → **353/353 أخضر**؛ E2E `open.spec.ts` **O1** («2/3» → «إجابة موفقة» + إتقان بلا تسريب DOM) و**O2** (توليد مفتوح عند الطلب) → **40/40 أخضر**.
- `npm run check` أخضر (353/353) + `npm run build` أخضر + docs sync (DECISIONS D-034/TASKS/CHANGELOG/TEST_PLAN/README/API_SPEC) + commit.

## 2026-09-26 — الجلسة الخامسة والعشرون (PHASE 31: زمن الإجابة في معادلة الإتقان + شارات الإتقان المتدرجة — إغلاق بندَي D-028/D-030 «إنتاج لاحقًا»)
- **القرار D-035**: معادلة الإتقان تستجيب الآن **لسرعة الإجابة** (بند D-028/D-030 «معادلة تشمل وقت الإجابة») وتتدرّج **شارات الإتقان** («متقن 3/5 مفاهيم» — بند D-030). زمن الإجابة «مضاعِف قوة إشارة» لا حُكم: سريع (<10ث) ×1.25، عادي (10..60) ×1، بطيء (>60) ×0.75، مجهول ×1 — يضخّم الدلتا باتجاه واحد للصح والخطأ (جواب واثق سريع إشارة قوية، متردّد إشارة ضعيفة). **المحاولة الأولى تبقى محايدة** (صحيح 0.6/خاطئ 0.1) مهما كانت السرعة، وغياب الزمن يبقي المسار القديم حرفيًا (الوحدة).
- **القياس عميلًا والتخزين خادمًا**: `PracticeState.shownAt` يُثبَّت عند كل سؤال جديد؛ على الإرسال يُحسب `Math.round((now−shownAt)/1000)` مقصوصًا 1..600 ويُرسل في **إجابات MCQ فقط** (`submitPracticeAnswer(id, option, timeTakenSeconds?)`) — المفتوح لا يُقاس (زمن الكتابة ليس إشارة إتقان). الخادم يتحقق ويُحيّد (`normalizeAnswerSeconds`: صحيح ∈[1,600] وإلا null) ويخزّن في عمود جديد `answers.answer_seconds` (migration `0008` ALTER TABLE واحدة)؛ `recordAssessment(answerSeconds?)` يطبّق `timeScaledDelta(difficulty, seconds)` على الصف الموجود مع `round2` للعرض النظيف.
- **المحرك النقي**: `progress/mastery.ts` — `AnswerTimeBand` + حدّا النطاق + `TIME_MULTIPLIER` + `answerTimeBand` + `timeScaledDelta` (كلها بلا I/O).
- **الشارات المتدرجة**: `mastery_three` «متقن 3 مفاهيم» + `mastery_five` «متقن 5 مفاهيم» على حدث `mastery_achieved` (نفس عدّاد المستوى المعروض «متقن») — المجموع 8 → **10** شارات؛ تظهر تلقائيًا في شاشة الإنجازات (مكتسبة أو مقفولة).
- **اختبارات**: وحدة `masteryTime.test.ts` **+8** + API `practiceTime.test.ts` **+4** (تخزين/تفاوت سريع>بطيء 0.79>0.71، حيادية المحاولة الأولى، إسقاط فاسد/خارج النطاق، صفر زمن على المفتوح) + API `tierBadges.test.ts` **+2** (3 ثم 5 عبر حلقة الإرسال الحقيقية) + تحديث total 8→10 → **367/367 أخضر**؛ E2E `achievements.spec.ts` E1 (+10 صفوف وشارتا التدرّج مقفلتان) → **40/40 أخضر**.
- `npm run check` أخضر (367/367) + `npm run build` أخضر + docs sync (DECISIONS D-035/TASKS/CHANGELOG/TEST_PLAN/README/API_SPEC) + commit.

## 2026-09-26 — الجلسة السادسة والعشرون (PHASE 32: التعاقب اليومي + شارات «المواظبة» — إغلاق بندَي D-024/D-035 «تتابع أسبوعي»)
- **القرار D-036**: شارة **«بقاء أسبوعي (تميّز تعاقبي)»** الموعودة في D-024 وD-035 تعمل الآن — أيام النشاط المتتالية تُكافأ بشارتين: «مواظب 3 أيام» و«مواظب أسبوع» (المجموع 10 → **12**). التعاقب يُحسب **قراءةً** من بيانات التمارين والجلسات نفسها (لا عدّاد متراكم ولا جدول جديد): يوم نشط = أي إجابة (`answers.createdAt`) أو بدء جلسة (`learning_sessions.startedAt`)، ويوم لم ينتهِ بعد لا يكسر السلسلة.
- **محرك نقي**: `progress/streak.ts` — `dayKey`/`previousDay`/`streakForDates` (UTC؛ تكرارات تُدمج، مستقبلية تُتجاهل).
- **الخدمة**: `AchievementService.streakForStudent` + `evaluateStreak` (منح بقياس **التعاقب الحالي** لا عدّاد تراكمي عبر نفس `awardFor` المضاد للتكرار؛ غياب يوم لا يلغي شارة مضت) + حدث `daily_streak` يفوضه `evaluate`.
- **نقاط الحقن best-effort**: بعد كل إجابة تمرين وفي كل حدث جلسة — التعاقب يُعاد تقييمه بتكلفة سفّليّة ولا يكسر مسار الجواب أبدًا.
- **الخطة والواجهة**: `GET /api/practice/plan` ← `{ plan, streak }`؛ Home يعرض «🔥 تعاقب N أيام» فوق خطة الممارسة (`data-testid="practice-streak"`، جمع عربي 1/2/3-10/11+).
- **درس تشخيصي**: جدول `learning_sessions` **بلا** `createdAt` (لديه `startedAt`)، وعمودا الزمن `ts()` = epoch ميلي-ثانية — استخراج اليوم يتم في JS من كائن `Date` (دالة SQLite `date()` تعثّرت مع مرجعَي جدولين).
- **اختبارات**: وحدة `streak.test.ts` **+10** + API `streak.test.ts` **+4** (3 أيام متتالية تمنح «مواظب 3 أيام» وتبقي «مواظب أسبوع» مقفولة + يومٌ ضائع يعيد التصفير (2 بلا شارة) + الجلسات نشاطٌ يُدمج أُيّامه + لا منح مزدوج عند إعادة التقييم) + إجمالات 10→12 → **381/381 أخضر**؛ E2E `achievements.spec.ts` E1 (12 صفًا + `streak_three`/`streak_seven` مقفلتان) → **40/40 أخضر**.
- `npm run check` أخضر (381/381) + `npm run build` أخضر + docs sync (DECISIONS D-036 + ذيلَي D-024/D-035/TASKS/CHANGELOG/TEST_PLAN/README/API_SPEC) + commit.

## 2026-09-26 — الجلسة السابعة والعشرون (PHASE 33: الجلسات في حلقة الإتقان — إنهاء جلسة درس يُنعش حداثة مفاصله، والشارات تبقى معلّقة على آخر ممارسة)
- **القرار D-037**: بعد مذاكرة درس، تعتبر خطة الممارسة والعرض مفاهيمه «طازجة» (كما في أنظمة SRS) — لكن **الشارة لا تُشترى بمطالعة**. عمود جديد `student_progress.last_practice_at` (migration `0009`) يفصل «آخر تعرّض» (`last_seen_at`) عن «آخر ممارسة فعلية» (يكتبه `recordAssessment` دائمًا).
- **إنهاء الجلسة** (`sessions/service.ts` end): كتلة best-effort تنعش `last_seen_at` فقط لمفاهيم `session.lessonId` ذات الصفوف القائمة عبر `touchConceptRecency` (بلا مساس بالإتقان/المحاولات، بلا صفوف جديدة، بلا فشل يُكسر الإنهاء).
- **الشارات المرتبطة بالممارسة**: عدّاد `mastery_achieved` يحسب الانحلال من `last_practiced_at ?? last_seen_at` — التعرّض بالجلسة وحده لا يمنح «متقن»؛ الصفوف القديمة (قبل العمود) تُعبَّأ بالتساوي في `applyMigrations` (`last_practice_at = last_seen_at WHERE null`).
- **إصلاح «المدوّن يمحو كتابة أحدث»** (`web/Chat.tsx`): عند اكتمال دور المعلم، يصفّر المُدوّن **فقط** إن كان ما زال يحمل النص المُرسل — نص كُتب أثناء انتظار الرد لا يُمحى (يزيل سباق E2E R1).
- **الخطة والواجهة**: لا تغيير في سطح API — أثر PHASE 33 يظهر في ترتيب الخطة وانحلال العرض: مذاكرة درس اليوم ترجّئ مفاهيمه في قائمة «راجع الآن» بدلًا من الإنذار بها.
- **اختبارات**: API `sessionRecency.test.ts` **+6** (إنعاش حداثة المفاهيم الممارَسة فقط بلا مساس بالإتقان + لا صفوف للمفاهيم غير الممارَسة وعزل الدروس الأخرى + التعرّض لا يمنح «متقن» بينما الممارسة تمنحه + صفوف قديمة `null` تنحل من `last_seen_at` + إنهاء بلا درس/بلا ممارسة no-op + إعادة ترتيب الخطة بعد مذاكرة) → **387/387 أخضر**؛ E2E `practice.spec.ts` **PR5** (درس → رسالة → إنهاء → الخطة سليمة) + تثبيت `recap.spec.ts` R1/R2 (انتظار عنوان الدرس الحقيقي بدلًا من «الجلسة» المؤقت) → **41/41 أخضر**.
- `npm run check` أخضر (387/387) + `npm run build` أخضر + docs sync (DECISIONS D-037/TASKS/CHANGELOG/TEST_PLAN/README/API_SPEC) + commit.
